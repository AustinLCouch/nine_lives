use bevy::prelude::*;
use nine_lives_ui::{PresetButton, SelectedPreset, setup_selected_preset, handle_preset_selection, sync_preset_button_highlights};
...
fn test_preset_button_highlighting_sync() {
    ...
    app.add_systems(Update, (handle_preset_selection, sync_preset_button_highlights));
    ...
    let cozy_kitten_idx = presets.iter().position(|p| matches!(p, PresetKind::CozyKitten)).unwrap();
    let curious_cat_idx = presets.iter().position(|p| matches!(p, PresetKind::CuriousCat)).unwrap();
    
    // Spawn test preset buttons manually
    let cozy_button = app.world_mut().spawn((
        Button,
        PresetButton { preset_id: cozy_kitten_idx },
        BackgroundColor(Color::srgb(0.2, 0.2, 0.3)), // Normal style initially
        BorderColor(Color::srgb(0.4, 0.4, 0.5)),
        Interaction::None,
    )).id();
    
    let curious_button = app.world_mut().spawn((
        Button, 
        PresetButton { preset_id: curious_cat_idx },
        BackgroundColor(Color::srgb(0.2, 0.2, 0.3)), // Normal style initially
        BorderColor(Color::srgb(0.4, 0.4, 0.5)),
        Interaction::None,
    )).id();
    ...
    let mut interaction = app.world_mut().get_mut::<Interaction>(curious_button).unwrap();
    ...
    *interaction = Interaction::None;
    ...
    assert_eq!(cozy_bg.0, expected_normal_bg, 
    ...
    assert_eq!(curious_bg.0, expected_selected_bg,
